...
class PlatformController:
    ...
    def optimize_offsets(self, trajectory_df: pd.DataFrame, num_cores: int = None) -> Tuple[np.ndarray, np.ndarray]:
        """
        Optimize position and rotation offsets to minimize peak accelerations
        while ensuring trajectory feasibility. Uses multicore processing.
        ...
        def optimize_with_initial_guess(x0: np.ndarray, bounds: List[Tuple[float, float]]) -> Dict:
            """Helper function to run optimization with a single initial guess"""
            try:
                result = minimize(
        ...
        with ProcessPoolExecutor(max_workers=num_cores) as executor:
            ...
            futures = [
                executor.submit(optimize_with_initial_guess, x0, bounds)
        ...
        if best_result is None:
            print("\nWarning: No valid solution found in any optimization attempt")
    ...
    def objective_function(self, params: np.ndarray, trajectory_df: pd.DataFrame) -> float:
        """
        Objective function for optimization. Separated from optimize_offsets for parallel processing.
        ...
        try:
            # Process trajectory with current offsets
            results_df = self.process_trajectory(trajectory_df, apply_optimization=True)
            
            # If results_df is empty, return penalty
            if len(results_df) == 0:
                return 1e6
            
            # Check if we have all required columns
            required_columns = ['slider_positions', 'velocities', 'accelerations']
            if not all(col in results_df.columns for col in required_columns):
                return 1e6
            
            # Calculate peak accelerations
            peak_accel = max(max(abs(a)) for a in results_df['accelerations'])
            
            # Calculate peak velocities
            peak_vel = max(max(abs(v)) for v in results_df['velocities'])
            
            # Add penalty if any z position is below 0
            min_z = min(row['z'] + pos_offset[2] for _, row in trajectory_df.iterrows())
            if min_z < 0:
                return 1e6  # Large penalty for invalid z positions
            
            # Add penalty for out-of-range slider positions
            for slider_pos in results_df['slider_positions']:
                if any(pos < 0 or pos > self.rail_max_travel for pos in slider_pos):
                    return 1e6  # Large penalty for out-of-range slider positions
            
            # Return weighted sum of peak acceleration and velocity
            return peak_accel + 0.5 * peak_vel
            
        except Exception as e:
            return 1e6  # Return large penalty for failed attempts
        finally:
            # Restore original offsets
            self.position_offset = orig_pos_offset
            self.rotation_offset = orig_rot_offset

    def process_trajectory_chunk(self, chunk_data: Tuple[pd.DataFrame, dict]) -> List[dict]:
        """
        Process a chunk of trajectory data. Helper function for parallel processing.
        
        Args:
            chunk_data: Tuple of (chunk_df, shared_data)
            
        Returns:
            List of result dictionaries for the chunk
        """
        chunk_df, shared_data = chunk_data
        results = []
        ...
        for _, row in chunk_df.iterrows():
            try:
                ...
                if shared_data['apply_optimization']:
                    position = np.array([
    ...
    def process_trajectory(self, trajectory_df: pd.DataFrame, apply_optimization: bool = True, num_cores: int = None) -> pd.DataFrame:
        ...
        chunks = [trajectory_df.iloc[i:i + chunk_size] for i in range(0, len(trajectory_df), chunk_size)]
        ...
        with ProcessPoolExecutor(max_workers=num_cores) as executor:
            # Create a list of futures
            futures = [
                executor.submit(self.process_trajectory_chunk, (chunk, shared_data))
                for chunk in chunks
            ]
